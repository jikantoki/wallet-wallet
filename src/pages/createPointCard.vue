...
v-card(
  ...
  v-card-actions
    p.ml-2(style="font-size: 1.3em") 新規ポイントカード登録
    v-spacer
    v-btn(
      text
      @click="$router.back()"
  ...
  export default {
    ...
    async mounted () {
      App.addListener('backButton', () => {
        if (this.scannerDialog) {
          ...
          this.editCard.name.length > 0
          || this.editCard.cardData.length > 0
    ...
    methods: {
      ...
      addPointCard (card: PointCard) {
        ...
        this.cards.pointCards.push({ ...card })
        Toast.show({ text: 'ポイントカードを保存しました' })
        this.$router.push('/')
      },
    },
  }
